...
function getLoginTicket() {
  var settings = {
    "async": false,
    "crossDomain": true,
    "url": "https://login.liu.se/cas/login?service=https%3A%2F%2Fse.timeedit.net%2Fweb%2Fliu%2Fdb1%2Ftimeedit%2FssoResponse",
    "method": "GET",
    "headers": {
      "upgrade-insecure-requests": "1",
      "user-agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/58.0.3029.110 Safari/537.36",
  ...
  var loginTicket;
  $.ajax(settings).done(function (output, status, xhr) {
    /*    console.log(output);
     console.log(xhr.getAllResponseHeaders());*/
    loginTicket = $(output).find("input[name='lt']").val();
    chrome.storage.sync.set({"loginTicket": loginTicket}, function() {
    });
  });
  return loginTicket;
}